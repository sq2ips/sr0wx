...
# "VC" : "%"
    ...
    def __init__(self, ICAO=None, at=(0, 0, 0), taf=None, address=None):
        ...
        _skyCnd = re.compile(
            "(SKC|FEW|SCT|BKN|OVC|NSC)(\d\d\d(VV\d\d\d)?)?(CB|TCU)?")
        ...
        for line in report:
            if _valid.search(line) is not None:
                ...
                for word in line.split(" "):
                    tl["period"] = (
                        tl["period"] + " " + " ".join((self.flatten(_period.findall(word)) or ""))).strip()
                    tl["wind"] = (
                        tl["wind"] + " " + " ".join((self.flatten(_wind.findall(word)) or ""))).strip()
                    tl["vis"] = (
                        tl["vis"] + " " + " ".join((self.flatten(_vis.findall(word)) or ""))).strip()
                    tl["weather"] = (
                        tl["weather"] + " " + " ".join((self.flatten(_weather.findall(word)) or ""))).strip()
                    tl["temp"] = (
                        tl["temp"] + " " + " ".join((self.flatten(_temp.findall(word)) or ""))).strip()
                    tl["press"] = (
                        tl["press"] + " " + " ".join((self.flatten(_press.findall(word)) or ""))).strip()
                    tl["skyCnd"] = (
                        tl["skyCnd"] + " " + " ".join((self.flatten(_skyCnd.findall(word)) or ""))).strip()
                    tl["cavok"] = (
                        tl["cavok"] + " " + " ".join((self.flatten(_cavok.findall(word)) or ""))).strip()
                    tl["nsw"] = (
                        tl["nsw"] + " " + " ".join((self.flatten(_nsw.findall(word)) or ""))).strip()
        ...
        TAF0 = self.tafData[0]
        keys = ["period", "wind", "vis", "weather",
                "skyCnd", "temp", "press", "cavok", "nsw"]
        ...
        for line in self.tafData:
            if "/" in line["period"].strip():
                ...
                fromDD, fromHH, toDD, toHH = (
                    int(l[0:2]), int(l[2:4]), int(l[5:7]), int(l[7:9]))
